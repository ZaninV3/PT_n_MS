...
##### Интервал, в котором практически все значения $X$
...
$$f(x) = \frac{1}{s \sqrt{2 \pi}} e^{- \frac{(x - \overline{x})^2}{2s^2}}$$
...
###### Чертеж полигона относительных частот $p_i^*$ и кривой распределения $p_i$
